...
vector<int> load_nodes(12, 0);
set<pair<int,int>> actual_load_nodes;
int currentNodes = 4;
int activeNodes = 4;
map<string, map<int, vector<int>>> locs;
set<pair<int, int>> act_nodes;
...
bool checker(vector<string> &inputLines, vector<string> &outputLines) {
    ...
    for (auto inp : inputLines) {
        ...
        if (command == "upload") {
            ...
            if(res == 1)
            {
                ...
                for (int i = 0; i < numChunks; i++) {
                    ...
                    for(int j = 0; j < numLocations; j++)
                    {
                        ...
                        tmp.push_back(*actual_load_nodes.begin());
                        actual_load_nodes.erase(actual_load_nodes.begin());
                        load_nodes[chunkData[2 + j]]++;
                        locs[fileName][chunkNumber].push_back(chunkData[2 + j]);
                    }

                    for(auto el: tmp)
                    {
                        actual_load_nodes.insert({el.first + 1, el.second});
                    }
                    m++;
                }
            }

            vector<int> lbvals_act;
            vector<int> lbvals_opt;

            for(auto it: actual_load_nodes) lbvals_opt.push_back(it.first);
            for(int j = 1; j < currentNodes; j++)
            {
                if(!nodes[j]) continue;
                lbvals_act.push_back(load_nodes[j]);
            }
            sort(lbvals_act.begin(), lbvals_act.end());
            sort(lbvals_opt.begin(), lbvals_opt.end());

            if(lbvals_act != lbvals_opt) 
            {
                return false;
            }

            actual_load_nodes.clear();

            for(int j = 1; j < currentNodes; j++)
            {
                if(!nodes[j]) continue;
                actual_load_nodes.insert({load_nodes[j], j});
            }
        } 
            ...
            nodes[rank] = 0;
            activeNodes--;
            actual_load_nodes.erase({load_nodes[rank], rank});
            ...
            nodes[rank] = 1;
            activeNodes++;
            actual_load_nodes.insert({load_nodes[rank], rank});
